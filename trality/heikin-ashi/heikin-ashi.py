...
@parameter(
    name="body_index", type="float", default=0, min=0.0, max=3.0, enabled=True
)  # body_index = 0.15 # [0.0, 0.3], Step = 0.01;
@parameter(
    name="trend_index", type="int", default=2, min=1, max=141, enabled=True
)  # trend_index = 80 # [1, 141], Step = 5;
...
def handler(state, data, params):
    ...
    if (ha_close[-1] > ha_open[-1]) and (ha_open[-1] - ha_low[-1]) <= (
        (ha_high[-1] - ha_low[-1]) * params.body_index
    ):
        if data.close[-1] > data.close[-1 - params.trend_index]:
            OpenLongTrade = True
    if (ha_close[-1] < ha_open[-1]) and (ha_high[-1] - ha_open[-1]) <= (
        (ha_high[-1] - ha_low[-1]) * params.body_index
    ):
        if data.close[-1] > data.close[-1 - params.trend_index]:
            OpenShortTrade = True
